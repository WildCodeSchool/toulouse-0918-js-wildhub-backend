...
const linkLanguages = (projectId, languageStat) => {
  ...
  return db.query(`SELECT iid, name from languages where name IN (${langNamesQuoted})`)
    .then(languages => db.query(
      query,
      Object.values(languageStat).reduce(
        (carry, quantity, index) => {
          const langIndex = languages.findIndex(lang => lang.name === langNames[index]);
          return carry.concat([projectId, languages[langIndex].iid, quantity]);
        }, []
      )
    ))
    .then(() => projectId);
