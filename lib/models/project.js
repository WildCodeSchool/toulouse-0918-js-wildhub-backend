...
const linkLanguages = (projectId, language_stat) => {
  const langNames = Object.keys(language_stat);
  ...
  return db.query(`SELECT id from languages where name IN (${langNames.map(l => "'" + l + "'")})`)
    .then(records => records.map(({ id }) => id))
    .then(languageIds => db.query(
      query,
      Object.values(language_stat).reduce(
...
const create = ({
  ...
  [id, owner.id, name, name, description, homepage, htmlUrl, language],
)
