...
const getAll = () => getProjectsWithLanguages(
  db.query('SELECT * FROM projects ORDER BY updatedAt DESC')
);
const getFromProjectIds = records => {
  ...
  return getProjectsWithLanguages(
    db.query(`SELECT * FROM projects WHERE iid IN (${projectIds}) ORDER BY updatedAt DESC`)
  );
};
...
const getAllByGitHubId = githubId => getProjectsWithLanguages(
  db.query(
    'SELECT * FROM projects WHERE ownerGithubId = ? ORDER BY updatedAt DESC', [githubId]
  )
);
